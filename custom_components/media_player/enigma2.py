...
from homeassistant.components.media_player import (
    ...
    MediaPlayerDevice, PLATFORM_SCHEMA, MEDIA_TYPE_TVSHOW, MEDIA_TYPE_VIDEO)
from homeassistant.const import (
    CONF_HOST, CONF_NAME, STATE_OFF, STATE_ON, STATE_UNKNOWN, CONF_PORT)
import homeassistant.helpers.config_validation as cv

REQUIREMENTS = ['openwebif.py>=0.8']

_LOGGER = logging.getLogger(__name__)

CONF_TIMEOUT = 'timeout'

DEFAULT_NAME = 'Enigma2 Media Player Remote'
DEFAULT_PORT = 80
DEFAULT_TIMEOUT = 0

# Support different items when watching recording playback
SUPPORT_ENIGMA2_RECORDING_PLAYBACK = SUPPORT_VOLUME_SET | SUPPORT_VOLUME_MUTE | \
...
class Enigma2Device(MediaPlayerDevice):
    ...
    def __init__(self, name, config):
        ...
        self._name = name
        self.e2_box = openwebif.api.CreateDevice(config['host'], port=config['port'])
    ...
    def supported_media_commands(self):
        ...
        if self.e2_box.get_current_playback_type(self.currservice_serviceref) == PlaybackType.recording:
    ...
    def media_title(self):
        ...
        if self.currservice_serviceref.startswith('1:0:0'):
            return "[Recording Playback] - " + self.current_programme_name
        if self.current_service_channel_name:
            return self.current_programme_name + " - " + self.current_service_channel_name
    ...
    def update(self):
        ...
        if self.e2_box.is_box_in_standby():
            ...
            self.current_service_channel_name = status_info['currservice_station']
            ...
            self.picon_url = self.e2_box.get_current_playing_picon_url(channel_name=self.current_service_channel_name,
